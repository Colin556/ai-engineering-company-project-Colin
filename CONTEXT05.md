...
Business Rules
current_stock is always computed, never stored. For any given ingredient: current_stock = SUM(IngredientEntry.quantity) − SUM(IngredientExit.quantity).
An exit cannot be logged if it would result in negative stock. Return HTTP 400 with the message: "Insufficient stock for ingredient '{name}'. Available: {available}, requested: {requested}.". Reject before writing.
Both Colombia and US ingredients exist in the same table. Use the country field to filter by market when needed.
No user table in Supabase. The user_uuid fields reference TinyDB users. Do not create a User model in SQLModel.
Locations are numeric IDs 1–14. They are not foreign keys in this milestone — store only the integer.
Seed Data
